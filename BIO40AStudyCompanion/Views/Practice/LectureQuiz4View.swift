...
struct LectureQuiz4View: View {
    ...
    private var filteredQuestions: [QuizQuestion] {
        let lq4 = content.allQuestions.filter { $0.chapterID == "lq4" }
        switch selectedMode {
        case .normal:
            return lq4.filter { $0.sectionID == "lq4_normal" }.shuffled()
        case .hard:
            return lq4.filter { $0.sectionID == "lq4_hard" }.shuffled()
        case .all:
            return lq4.shuffled()
        }
    }
    ...
    var body: some View {
        ...
        .navigationTitle("Lecture Quiz 4 Practice")
        .navigationDestination(isPresented: $startQuiz) {
            QuizView(questions: filteredQuestions, chapterTitle: "Lecture Quiz 4")
        }
    }
}
